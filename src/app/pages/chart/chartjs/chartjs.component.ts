import { Component, OnInit } from '@angular/core';

import { lineAreaChart, lineBarChart, pieChart, donutChart, radarChart, reportChart } from './data';

import { ChartType } from './chartjs.model';

@Component({
  selector: 'app-chartjs',
  templateUrl: './chartjs.component.html',
  styleUrls: ['./chartjs.component.scss']
})

/**
 * Chartjs component - handling the chartjs with navbar and content
 */
export class ChartjsComponent implements OnInit {

  // bread crumb data
  breadCrumbItems: Array<{}>;

  // Line Chart
  lineAreaChart: ChartType;
  // Bar Chart
  lineBarChart: ChartType;
  // Pie Chart
  pieChart: ChartType;
  // Donut Chart
  donutChart: ChartType;
  // Polar area Chart
  polarAreaChart: ChartType;
  // Radar Chart
  radarChart: ChartType;
  // Financial Report
  reportChart: ChartType;

  constructor() { }

  ngOnInit() {
    this.breadCrumbItems = [{ label: 'Minton', path: '/' }, { label: 'Charts', path: '/' }, { label: 'Chartjs', path: '/', active: true }];

    /**
     * fetches data
     */
    this._fetchData();
  }

  private _fetchData() {
    // Line Chart data
    this.lineAreaChart = lineAreaChart;
    // Bar Chart data
    this.lineBarChart = lineBarChart;
    // Pie Chart data
    this.pieChart = pieChart;
    // Donut Chart
    this.donutChart = donutChart;

    // Radar Chart data
    this.radarChart = radarChart;
    // Financial Report
    this.reportChart = reportChart;
  }

}
